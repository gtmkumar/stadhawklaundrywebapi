    ...
    public class DeliveryBoyController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post([FromForm] UsersViewModel users)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    if (!(await _unit.IUser.Exists(t => t.PhoneNumber.Equals(users.ContactNo) && t.UserType == 2)).UserObject)
                    {
                    ...
                    if ((await _unit.IUser.Exists(t => t.Email.Equals(users.EmailId) && t.UserType == 2)).UserObject)
                    {
